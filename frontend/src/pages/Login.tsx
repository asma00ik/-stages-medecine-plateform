...
const Login = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validate student matricule format if student role
    if (selectedRole === "student" && !/^\d{12}$/.test(identifier)) {
      toast({
        title: "Format invalide",
        description: "Le matricule doit contenir 12 chiffres (ex: 202231269705)",
        variant: "destructive",
      });
      setIsLoading(false);
      return;
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="identifier">
                  {selectedRole === "student" ? "Matricule" : "Identifiant / Email"}
                </Label>
                <Input
                  id="identifier"
                  type={selectedRole === "student" ? "text" : "email"}
                  placeholder={selectedRole === "student" ? "202231269705" : "votre.email@exemple.com"}
                  value={identifier}
                  onChange={(e) => setIdentifier(e.target.value)}
                  required
                  className="h-11"
                  maxLength={selectedRole === "student" ? 12 : undefined}
                />
                {selectedRole === "student" && (
                  <p className="text-xs text-muted-foreground">
                    Format: 4 chiffres année de bac + 8 chiffres d'identification
                  </p>
                )}
              </div>
